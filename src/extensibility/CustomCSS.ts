...
export type ConfigStyleInput =
  | string
  | CustomStyleConfig
  | Array<string | CustomStyleConfig>
...
export class CustomCSS {
  ...
  _config: CustomCSSConfig;
  _styles: Map<string, CustomStyleRecord>;
   ...
   */
  constructor(shadowRoot: HTMLElement | ShadowRoot, config: Partial<CustomCSSConfig> = {}) {
   ...
   */
  add(id: string, css: string, options: { scope?: string; mode?: string; priority?: number } = {}) {
   ...
   * @returns {string}
   */
  _addScopePrefix(css: string, scope: string) {
   ...
   */
  applyFromConfig(styles: ConfigStyleInput) {
    ...
    if (Array.isArray(styles)) {
      styles.forEach((css, index) => {
        if (typeof css === 'string') {
          ...
          const styleConfig = css as CustomStyleConfig;
          if (typeof styleConfig.css === 'string') {
            this.add(`config-${styleConfig.id || index}`, styleConfig.css, styleConfig);
          }
        }
      });
      return;
    ...
    if (styles && typeof styles === 'object') {
      ...
      for (const [scope, css] of Object.entries(styleMap)) {
        if (CSS_SCOPES[scope.toUpperCase() as keyof typeof CSS_SCOPES] && typeof css === 'string') {
          this.add(`config-${scope}`, css, { scope });
        } else {
          // Это объект стиля
          const styleConfig = styles as CustomStyleConfig;
          if (typeof styleConfig.css === 'string') {
            this.add(`config-${styleConfig.id || 'main'}`, styleConfig.css, styleConfig);
          }
          break;
        }
      }
   ...
   * @returns {string}
   */
  static fromObject(styleObject: Record<string, Record<string, string | number>>) {
    const rules = [];

 ...
 */
type CSSDeclarationValue = string | number;
type CSSDeclarationMap = Record<string, CSSDeclarationValue>;
type HoverSelectorStyles = Record<string, CSSDeclarationValue | CSSDeclarationMap>;
type HoverEffectDefinition = {
  category: string;
  name: string;
  description: string;
  styles: HoverSelectorStyles;
  interactive?: boolean;
...
type HoverEffectConfig = {
  effect?: string;
  duration?: number;
  customStyles?: CSSDeclarationMap;
  max?: number;
  perspective?: number;
  scale?: number;
};

export const HOVER_EFFECTS: Record<string, HoverEffectDefinition> = {
  // =====================
...
export class HoverEffects {
   ...
   */
  constructor(element: HTMLElement, config: HoverEffectConfig = {}) {
   ...
   * @returns {Object}
   */
  getEffect(): HoverEffectDefinition {
  ...
  apply() {
    ...
    for (const [property, value] of Object.entries(effect.styles)) {
      if (!property.startsWith(':') && typeof value !== 'object') {
        this._element.style.setProperty(property.replace(/([A-Z])/g, '-$1').toLowerCase(), String(value));
      }
    }
   ...
   * @param {Object} effect 
   */
  _injectStyles(effect: HoverEffectDefinition) {
    if (this._styleElement) {
      this._styleElement.remove();
    }

    const hoverStyles = this._getSelectorStyles(effect.styles[':hover']);
    const beforeStyles = this._getSelectorStyles(effect.styles[':hover::before']);
    const afterStyles = this._getSelectorStyles(effect.styles[':hover::after']);
    ...
    // ::before стили
    const baseBeforeStyles = this._getSelectorStyles(effect.styles['::before']);
    if (Object.keys(baseBeforeStyles).length > 0 || Object.keys(beforeStyles).length > 0) {
      css += `.${this._effectId}::before { content: ''; ${this._objectToCSS(baseBeforeStyles)} }\n`;
   ...
   * @returns {string}
   */
  _objectToCSS(obj: CSSDeclarationMap) {
  ...
  _getSelectorStyles(value: CSSDeclarationValue | CSSDeclarationMap | undefined): CSSDeclarationMap {
   ...
   */
  static applyTilt(element: HTMLElement, options: HoverEffectConfig = {}) {

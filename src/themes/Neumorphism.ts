...
type NeumorphismType = typeof NEUMORPHISM_TYPES[keyof typeof NEUMORPHISM_TYPES];
type NeumorphismIntensityLevel = typeof INTENSITY_LEVELS[keyof typeof INTENSITY_LEVELS];

type NeumorphismConfig = {
  type?: NeumorphismType;
  intensity?: NeumorphismIntensityLevel;
  palette?: string;
  lightAngle?: number;
  borderRadius?: number;
};

type ResolvedNeumorphismConfig = {
  type: NeumorphismType;
  intensity: NeumorphismIntensityLevel;
  palette: string;
  lightAngle: number;
...
export const NEUMORPHISM_PALETTES: Record<string, NeumorphismPalette> = {
  // Светлая тема
  LIGHT: {
    background: '#e0e5ec',
    lightShadow: '#ffffff',
...
export class Neumorphism {
  _config: ResolvedNeumorphismConfig;

  /**
   * @param {Object} config - Конфигурация
   */
  constructor(config: NeumorphismConfig = {}) {
    this._config = {
   ...
   */
  setIntensity(intensity: NeumorphismIntensityLevel) {
  ...
  _calculateShadowOffset() {
    ...
    const intensity = INTENSITY_CONFIG[this._config.intensity] as NeumorphismIntensity;
   ...
   * @returns {string}
   */
  generateShadow(overrides: NeumorphismConfig = {}) {
    const config: ResolvedNeumorphismConfig = { ...this._config, ...overrides };
    const palette = this.getPalette();
    const intensity = INTENSITY_CONFIG[config.intensity] as NeumorphismIntensity;
   ...
   * @returns {Object}
   */
  generateStyles(overrides: NeumorphismConfig = {}): NeumorphismStyles {
    const palette = this.getPalette();
    const shadow = this.generateShadow(overrides);
    const gradient = this.generateGradient();

    const styles: NeumorphismStyles = {
   ...
   * @param {Object} overrides 
   */
  apply(element: HTMLElement, overrides: NeumorphismConfig = {}) {
    const styles = this.generateStyles(overrides);
    
    for (const [property, value] of Object.entries(styles)) {
      element.style.setProperty(property.replace(/[A-Z]/g, (match) => `-${match.toLowerCase()}`), String(value));
    }

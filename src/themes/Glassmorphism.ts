...
type GlassmorphismConfig = {
  preset?: string;
  colorScheme?: string;
  customSettings?: Partial<GlassPreset>;
};
 ...
 */
export const GLASS_COLORS: Record<string, GlassColorScheme> = {
  LIGHT: {
...
export class Glassmorphism {
   ...
   */
  generateStyles(overrides: Partial<GlassPreset> = {}): GlassStyleMap {
    const settings = { ...this.getSettings(), ...overrides };
    const colors = GLASS_COLORS[this._config.colorScheme] || GLASS_COLORS.LIGHT;

    const background = colors.background
      .replace('{opacity}', String(settings.opacity));
    
    const border = colors.border
      .replace('{borderOpacity}', String(settings.borderOpacity));
    
    const shadow = colors.shadow
      .replace('{shadowOpacity}', String(settings.shadowOpacity));

    const styles: GlassStyleMap = {
      background,
   ...
   */
  apply(element: HTMLElement, overrides: Partial<GlassPreset> = {}) {
   ...
   * @returns {string}
   */
  generateCSS(className = 'glass', overrides: Partial<GlassPreset> = {}) {
    const styles = this.generateStyles(overrides);

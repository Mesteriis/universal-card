...
type PatternRenderer = (color?: string, size?: number) => string;
...
export class BackgroundPatterns {
  _config: BackgroundPatternConfig;

  /**
   * @param {Object} config - Конфигурация
   */
  constructor(config: BackgroundPatternConfig = {}) {
    this._config = {
      pattern: 'grid',
   ...
   * @returns {Object}
   */
  generateStyles(overrides: BackgroundPatternConfig = {}): Partial<BackgroundPatternStyles> {
    ...
    const styles: Partial<BackgroundPatternStyles> = {
      backgroundBlendMode: String(config.blend || 'normal')
    };
   ...
   * @param {Object} overrides 
   */
  apply(element: HTMLElement, overrides: BackgroundPatternConfig = {}) {
   ...
   * @returns {Object}
   */
  static getByCategory(category: PatternCategory): Record<string, PatternDefinition> {

...
export type ImageSourceType = (typeof SOURCE_TYPES)[keyof typeof SOURCE_TYPES];
export type ImageClickAction = 'more-info' | 'fullscreen' | 'none';

export interface ImageEntityConfig {
  entity_id: string;
  url: string;
  fit: ImageFit;
  aspect_ratio: string;
  ...
  lazy_load: boolean;
  click_action: ImageClickAction;
  border_radius: number;
}

type ImageEntityProviders = ProviderContext;
 ...
 */
const DEFAULT_CONFIG: ImageEntityConfig = {
  entity_id: '',
  url: '',
...
export class ImageEntity {
  _providers: ImageEntityProviders;
  _config: ImageEntityConfig;
   ...
   */
  constructor(providerContext: ProviderSource, config: Partial<ImageEntityConfig> = {}) {
   ...
   * @param {Object} config 
   */
  setConfig(config: Partial<ImageEntityConfig>) {
    this._config = { ...DEFAULT_CONFIG, ...config };
  }
  ...
  render(): HTMLElement {
    ...
    this._imgElement = this._element.querySelector('.uc-image') as HTMLImageElement | null;
   ...
   * @returns {string}
   */
  _getSourceType(): ImageSourceType | null {
  ...
  _getImageUrl(): string | null {
    ...
    switch (sourceType) {
      ...
      case SOURCE_TYPES.CAMERA:
        // Предпочитаем entity_picture из состояния, fallback на camera proxy.
        return this._providers.entities.getState(entity_id)?.attributes?.entity_picture || `/api/camera_proxy/${entity_id}`;
        
      case SOURCE_TYPES.IMAGE:
        // Image entity содержит URL в атрибутах
        const imageState = this._providers.entities.getState(entity_id);
        return imageState?.attributes?.entity_picture || imageState?.attributes?.url;
        
      case SOURCE_TYPES.PERSON:
        const personState = this._providers.entities.getState(entity_id);
   ...
   */
  _loadImage() {
    if (!this._imgElement || !this._element) {
      return;
    }
  ...
  _updateOverlays() {
    ...
    const state = this._providers.entities.getState(entity_id);
    if (!state) return;
   ...
   */
  _onImageLoad() {
    if (!this._element || !this._imgElement) {
      return;
    }
    ...
    const placeholder = this._element.querySelector('.uc-image-placeholder') as HTMLElement | null;
    if (placeholder) {
  ...
  _showFullscreen() {
    ...
    overlay.addEventListener('click', (e) => {
      ...
      if (target === overlay || target?.classList.contains('uc-image-fullscreen-close')) {
        overlay.remove();
      }
    });

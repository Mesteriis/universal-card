...
type NotificationCenterConfig = Record<string, any>;
type NotificationCenterProviders = ReturnType<typeof normalizeProviderContext>;
...
export class NotificationCenter {
  _providers: NotificationCenterProviders;
  _config: NotificationCenterConfig;
  _element: HTMLElement | null;
  _notifications: NotificationRecord[];
  _dismissedIds: Set<string>;
  _unsubscribe: (() => void) | null;
  _subscriptionEpoch: number;
  /**
   * @param {Object} providerContext - Provider context или raw hass
   * @param {Object} config - Конфигурация
   */
  constructor(providerContext: ProviderSource, config: NotificationCenterConfig = {}) {
  ...
  async _loadNotifications() {
    try {
      const persistentNotifications = await this._providers.notifications.list();

      let notifications: NotificationRecord[] = [];
      
      if (Array.isArray(persistentNotifications)) {
        notifications = persistentNotifications.map(n => ({
          id: n.id,
          title: n.title,
          message: n.message,
          created_at: new Date(n.created_at),
          source: n.source || 'persistent_notification',
   ...
   */
  _subscribeToUpdates() {
    this._subscriptionEpoch += 1;
    const currentEpoch = this._subscriptionEpoch;
    ...
    Promise.resolve(this._providers.notifications.subscribe(() => {
      void this._loadNotifications();
    }))
      .then((unsubscribe) => {
        if (currentEpoch !== this._subscriptionEpoch) {
      ...
      .catch((error) => {
        console.warn('[NotificationCenter] Notification subscription failed:', error);
      });
  }
  ...
  _renderNotifications() {
    ...
    const grouped: Record<string, NotificationRecord[]> = this._config.group_by_source
  ...
  _bindEvents() {
    ...
    this._element.querySelectorAll<HTMLElement>('[data-action="dismiss"]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const notifEl = btn.closest('.uc-notification') as HTMLElement | null;
    ...
    // Клик на уведомление
    this._element.querySelectorAll<HTMLElement>('.uc-notification').forEach(notif => {
  ...
  async _dismissNotification(id) {
    try {
      await this._providers.notifications.dismiss(id);
    } catch (e) {
    ...
    // Анимация удаления
    const el = this._element.querySelector<HTMLElement>(`[data-id="${id}"]`);
    if (el) {
  ...
  async _clearAll() {
    for (const n of this._notifications) {
      if (n.dismissible) {
        try {
          await this._providers.notifications.dismiss(n.id);
        } catch (e) {}

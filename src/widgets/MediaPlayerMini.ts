...
export type MediaPlayerDisplayStyle = (typeof DISPLAY_STYLES)[keyof typeof DISPLAY_STYLES];

export interface MediaPlayerMiniConfig {
  entity_id: string;
  style: MediaPlayerDisplayStyle;
  show_artwork: boolean;
  show_volume: boolean;
  show_progress: boolean;
  show_source: boolean;
  artwork_border_radius: number;
  volume_step: number;
}

type MediaPlayerMiniProviders = ProviderContext;
 ...
 */
const DEFAULT_CONFIG: MediaPlayerMiniConfig = {
  entity_id: '',
...
export class MediaPlayerMini {
  _providers: MediaPlayerMiniProviders;
  _config: MediaPlayerMiniConfig;
  _element: HTMLElement | null;
  _progressInterval: number | null;
  /**
   * @param {Object} providerContext - Provider context или raw hass
   * @param {Object} config - Конфигурация
   */
  constructor(providerContext: ProviderSource, config: Partial<MediaPlayerMiniConfig> = {}) {
   ...
   * @param {Object} config 
   */
  setConfig(config: Partial<MediaPlayerMiniConfig>) {
    this._config = { ...DEFAULT_CONFIG, ...config };
  }
   ...
   * @returns {Object|null}
   */
  _getState(): HomeAssistantEntityState | null {
    return this._providers.entities.getState(this._config.entity_id);
  }
  ...
  _bindEvents() {
    ...
    // Volume slider
    const volumeSlider = this._element.querySelector('.uc-media-volume-slider') as HTMLInputElement | null;
    if (volumeSlider) {
      volumeSlider.addEventListener('input', (e) => {
        this._setVolume(parseInt((e.target as HTMLInputElement | null)?.value || '0', 10) / 100);
      });
    }

    // Source select
    const sourceSelect = this._element.querySelector('.uc-media-source-select') as HTMLSelectElement | null;
    if (sourceSelect) {
      sourceSelect.addEventListener('change', (e) => {
        this._selectSource((e.target as HTMLSelectElement | null)?.value);
      });
    }

    // Progress bar click
    const progressBar = this._element.querySelector('.uc-media-progress-bar') as HTMLElement | null;
    if (progressBar) {
  ...
  _handleAction(action?: string) {
    ...
    switch (action) {
      case 'play_pause':
        this._providers.services.call('media_player', 'media_play_pause', { entity_id });
        break;
      case 'prev':
        this._providers.services.call('media_player', 'media_previous_track', { entity_id });
        break;
      case 'next':
        this._providers.services.call('media_player', 'media_next_track', { entity_id });
        break;
      case 'volume_mute':
        this._providers.services.call('media_player', 'volume_mute', { 
          entity_id,
  ...
  _setVolume(level: number) {
    this._providers.services.call('media_player', 'volume_set', {
   ...
   * @param {string} source 
   */
  _selectSource(source?: string) {
    this._providers.services.call('media_player', 'select_source', {
      entity_id: this._config.entity_id,
      source
    });
  }
   ...
   */
  _seek(percent: number) {
    const state = this._getState();
    const duration = state?.attributes?.media_duration;
    
    if (duration) {
      this._providers.services.call('media_player', 'media_seek', {
        entity_id: this._config.entity_id,
   ...
   * @returns {number}
   */
  _getProgressPercent(attrs: HomeAssistantEntityAttributes) {
  ...
  _updateProgress() {
    ...
    const progressFill = this._element.querySelector('.uc-media-progress-fill') as HTMLElement | null;
    if (progressFill) {

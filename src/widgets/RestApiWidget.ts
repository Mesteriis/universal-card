...
type RestApiWidgetConfig = Record<string, any>;
type RestApiWidgetProviders = ReturnType<typeof normalizeProviderContext>;
...
export class RestApiWidget {
  _providers: RestApiWidgetProviders;
  _config: RestApiWidgetConfig;
   ...
   */
  constructor(providerContext: ProviderSource, config: RestApiWidgetConfig = {}) {
  ...
  async _fetchData() {
    ...
    try {
      // Выполняем запрос
      const response = await this._providers.http.request(processedUrl, {
        method,
        headers: requestHeaders,
      ...
      this._error = didTimeout 
        ...
        : getErrorMessage(error, this._config.error_message);
  ...
  _processTemplate(input) {
    if (typeof input === 'string') {
      ...
      return input.replace(/\{\{\s*(\w+\.\w+)\s*\}\}/g, (match, entityId) => {
        return this._providers.entities.getState(entityId)?.state || match;
      });
    }
        ...
        ${items.map(item => {
          if (typeof item === 'object') {
            if (item.key !== undefined) {
              return `<li><strong>${this._escapeHtml(item.key)}:</strong> ${this._formatValue(item.value)}</li>`;
            }

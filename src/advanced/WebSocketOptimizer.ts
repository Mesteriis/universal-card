...
type WebSocketOptimizerConfig = Record<string, any>;
...
type PendingUpdateState = Record<string, any>;
...
class WebSocketOptimizerClass {
  _config: WebSocketOptimizerConfig;
  _subscribers: Map<string, Set<WebSocketSubscription>>;
  _pendingUpdates: Map<string, PendingUpdateState>;
  _batchTimeout: number | null;
  _lastUpdateTime: Map<string, number>;
  _visibilityObserver: IntersectionObserver | null;
  _visibleElements: WeakSet<HTMLElement>;
   ...
   * @param {Object} config 
   */
  configure(config: WebSocketOptimizerConfig) {
   ...
   */
  subscribe(entityId: string, callback: (state: Record<string, any>) => void, options: Record<string, any> = {}) {
    const subscription = {
      callback,
   ...
   */
  _unsubscribe(entityId: string, subscription: WebSocketSubscription) {
  ...
  processHassUpdate(oldHass, newHass) {
    ...
    const changedEntities: Array<{ entityId: string; newState: Record<string, any> }> = [];
   ...
   */
  _processImmediate(changedEntities: Array<{ entityId: string; newState: Record<string, any> }>) {
   ...
   * @param {Array} changedEntities 
   */
  _processThrottled(changedEntities: Array<{ entityId: string; newState: Record<string, any> }>) {
    const now = Date.now();
   ...
   * @param {Array} changedEntities 
   */
  _processBatched(changedEntities: Array<{ entityId: string; newState: Record<string, any> }>) {
    for (const { entityId, newState } of changedEntities) {
      this._pendingUpdates.set(entityId, newState);
    }
  ...
  _processBatch() {
    ...
    const entries = (Array.from(this._pendingUpdates.entries()) as Array<[string, Record<string, any>]>)
   ...
   * @param {number} delay 
   */
  _scheduleDeferredUpdate(entityId: string, delay: number) {
    setTimeout(() => {
   ...
   * @param {Object} newState 
   */
  _notifySubscribers(entityId: string, newState: Record<string, any>) {
    const subs = this._subscribers.get(entityId);
    if (!subs) return;
   ...
   */
  _observeVisibility(element: HTMLElement) {
    if (!this._visibilityObserver) {
      this._visibilityObserver = new IntersectionObserver(
        (entries) => {
          for (const entry of entries) {
            if (entry.isIntersecting) {
              this._visibleElements.add(entry.target as HTMLElement);
            } else {
              this._visibleElements.delete(entry.target as HTMLElement);
            }
          }
        },
   ...
   * @param {HTMLElement} element 
   */
  _unobserveVisibility(element: HTMLElement) {
    if (this._visibilityObserver) {
   ...
   * @param {Object} state 
   */
  forceUpdate(entityId: string, state: Record<string, any>) {

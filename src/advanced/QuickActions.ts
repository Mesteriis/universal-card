...
type QuickActionRecord = Record<string, any> & {
  id: string;
  entity_id?: string;
};
type QuickActionsConfig = Record<string, any>;
type QuickActionsProviders = ReturnType<typeof normalizeProviderContext>;
...
export class QuickActions {
  _providers: QuickActionsProviders;
  _config: QuickActionsConfig;
  _actions: QuickActionRecord[];
  _element: HTMLElement | null;
  _confirmDialog: HTMLElement | null;
  /**
   * @param {Object} providerContext - Provider context или raw hass
   * @param {Object} config - Конфигурация
   */
  constructor(providerContext: HomeAssistantLike, config: QuickActionsConfig = {}) {
  ...
  _executeToggle(action) {
    ...
    this._providers.services.call(domain, service, {
      entity_id: action.entity_id
    });
  }
  ...
  _executeTurnOn(action) {
    ...
    const domain = action.entity_id.split('.')[0];
    this._providers.services.call(domain, 'turn_on', {
      entity_id: action.entity_id,
      ...action.service_data
    });
  }
  ...
  _executeTurnOff(action) {
    ...
    const domain = action.entity_id.split('.')[0];
    this._providers.services.call(domain, 'turn_off', {
      entity_id: action.entity_id
    });
  }
  ...
  _executeService(action) {
    ...
    if (action.target) {
      this._providers.services.call(domain, service, serviceData, action.target);
    } else if (action.entity_id) {
      serviceData.entity_id = action.entity_id;
      this._providers.services.call(domain, service, serviceData);
    } else {
      this._providers.services.call(domain, service, serviceData);
    }
  }
  ...
  _executeFireEvent(action) {
    ...
    this._providers.api.call('POST', 'events/' + action.event, action.event_data || {});
  }
  ...
  _executeScript(action) {
    ...
    this._providers.services.call('script', 'turn_on', {
      entity_id: action.entity_id,
      ...action.service_data
    });
  }
  ...
  _executeScene(action) {
    ...
    this._providers.services.call('scene', 'turn_on', {
      entity_id: action.entity_id
    });
  }
  ...
  async _showConfirmation(confirmation) {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        (dialog.querySelector('.uc-confirm-ok') as HTMLElement | null)?.focus();
      }, 10);
    });
  }
  ...
  _isActionActive(action) {
    ...
    const state = this._providers.entities.getState(action.entity_id);
  ...
  _evaluateCondition(condition) {
    ...
    if (typeof condition === 'string') {
      const state = this._providers.entities.getState(condition);
      return state?.state === 'on';
    }

    // Объект условия
    if (condition.entity_id) {
      const state = this._providers.entities.getState(condition.entity_id);
      if (!state) return false;
  ...
  _getEntityState(entityId) {
    const state = this._providers.entities.getState(entityId);
    if (!state) return '-';
  ...
  _updateStates() {
    ...
    for (const action of this._actions) {
      ...
      buttonEl.classList.toggle('disabled', isDisabled);
    }
  }

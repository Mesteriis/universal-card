...
type TimerConfig = Record<string, any>;
type TimerProviders = ReturnType<typeof normalizeProviderContext>;
 ...
 */
export class Timer {
  _providers: TimerProviders;
  _config: TimerConfig;
  _element: HTMLElement | null;
  _intervalId: number | null;
  _targetTime: Date | null;
  _startTime: Date | null;
  _isRunning: boolean;
  _isPaused: boolean;
  _remainingOnPause: number;
  /**
   * @param {Object} providerContext - Provider context или raw hass
   * @param {Object} config - Конфигурация таймера
   */
  constructor(providerContext: HomeAssistantLike, config: TimerConfig = {}) {
   ...
   */
  _bindControlEvents() {
    const startBtn = this._element.querySelector('.uc-timer-start') as HTMLElement | null;
    const pauseBtn = this._element.querySelector('.uc-timer-pause') as HTMLElement | null;
  ...
  start() {
    ...
    if (this._config.mode === TIMER_MODES.TIMER && this._config.entity_id) {
      this._providers.services.call('timer', 'start', {
  ...
  pause() {
    ...
    // Для timer entity
    if (this._config.mode === TIMER_MODES.TIMER && this._config.entity_id) {
      this._providers.services.call('timer', 'pause', {
        entity_id: this._config.entity_id
      });
      return;
    }
  ...
  reset() {
    // Для timer entity
    if (this._config.mode === TIMER_MODES.TIMER && this._config.entity_id) {
      this._providers.services.call('timer', 'cancel', {
        entity_id: this._config.entity_id
      });
      return;
    }
  ...
  _executeEndAction() {
    ...
    if (action.service) {
      ...
      if (domain && service) {
        this._providers.services.call(domain, service, action.service_data || {});
      }
    }
  }
  ...
  _getRemaining() {
    switch (this._config.mode) {
      case TIMER_MODES.COUNTDOWN:
        ...
        return Math.max(0, Math.floor((this._targetTime.getTime() - Date.now()) / 1000));
  ...
  _getRemainingFromEntity() {
    ...
    if (entityId.startsWith('timer.')) {
      ...
      return Math.max(0, Math.floor((finishTime.getTime() - Date.now()) / 1000));
    }

...
type EntityPreviewConfig = Record<string, any>;
type EntityPreviewProviders = ReturnType<typeof normalizeProviderContext>;
type HistoryPoint = Record<string, any>;
type HistoryCacheEntry = {
  data: HistoryPoint[];
  timestamp: number;
};
type DebouncedShow = ReturnType<typeof debounce>;
...
export class EntityPreview {
  _providers: EntityPreviewProviders;
  _config: EntityPreviewConfig;
  _previewElement: HTMLElement | null;
  _currentEntityId: string | null;
  _historyCache: Map<string, HistoryCacheEntry>;
  _hideTimeout: number | null;
  _debouncedShow: DebouncedShow;
  /**
   * @param {Object} providerContext - Provider context или raw hass
   * @param {Object} config - Конфигурация превью
   */
  constructor(providerContext: HomeAssistantLike, config: EntityPreviewConfig = {}) {
  ...
  async _fetchHistory(entityId) {
    ...
    try {
      ...
      const response = await this._providers.api.call(
        'GET',
  ...
  _renderStats(history) {
    ...
    const statsEl = this._previewElement.querySelector('.uc-preview-stats') as HTMLElement | null;
    if (statsEl) {

...
type ResizeCallbackMap = {
  onStart: ((payload: ResizeEventPayload) => void) | null;
  onResize: ((payload: ResizeEventPayload) => void) | null;
  onEnd: ((payload: ResizeEventPayload) => void) | null;
};
...
type ResizeHandleListeners = {
  mouseDown: (e: MouseEvent) => void;
  touchStart: (e: TouchEvent) => void;
};

type ResizableCardsConfig = {
  handles: string[];
  minWidth: number;
  minHeight: number;
  maxWidth: number;
...
export class ResizableCards {
  ...
  _config: ResizableCardsConfig;
  _handles: Map<string, HTMLElement>;
  _isResizing: boolean;
  _currentHandle: string | null;
  _startRect: DOMRect | null;
  _startPos: ResizePosition;
  _callbacks: ResizeCallbackMap;
  _boundHandlers: ResizeHandlers;
  _handleListeners: Map<HTMLElement, ResizeHandleListeners>;

  /**
   * @param {HTMLElement} element - Элемент для resize
   * @param {Object} config - Конфигурация
   */
  constructor(element: HTMLElement, config: Partial<ResizableCardsConfig> = {}) {
   ...
   * @param {string} handle 
   */
  _applyHandleStyles(handleEl: HTMLElement, handle: string) {
  ...
  _bindEvents() {
    for (const [handle, handleEl] of this._handles) {
      const mouseDown = (e) => this._onMouseDown(e, handle);
      const touchStart = (e) => this._onTouchStart(e, handle);
      this._handleListeners.set(handleEl, { mouseDown, touchStart });
   ...
   */
  on(event: keyof ResizeCallbackMap, callback: ResizeCallbackMap[keyof ResizeCallbackMap]) {
   ...
   * @param {string} handle 
   */
  _onMouseDown(e: MouseEvent, handle: string) {
    e.preventDefault();
    e.stopPropagation();
   ...
   * @param {string} handle 
   */
  _onTouchStart(e: TouchEvent, handle: string) {
   ...
   * @param {number} y 
   */
  _startResize(handle: string, x: number, y: number) {
   ...
   * @param {number} y 
   */
  _handleResize(x: number, y: number) {
    const startRect = this._startRect;
    if (!startRect) return;
  ...
  destroy() {
    for (const handleEl of this._handles.values()) {
      const listeners = this._handleListeners.get(handleEl);
      if (listeners) {

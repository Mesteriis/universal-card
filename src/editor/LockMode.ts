...
type LockModeCallbacks = {
  onLock: ((level: string, actions: string[]) => void) | null;
  onUnlock: (() => void) | null;
  onAttempt: ((action: string) => void) | null;
};
...
export class LockMode {
  ...
  _config: LockModeConfig;
  _isLocked: boolean;
  _indicator: HTMLElement | null;
  _callbacks: LockModeCallbacks;

  /**
   * @param {HTMLElement} element - Элемент карточки
   * @param {Object} config - Конфигурация
   */
  constructor(element: HTMLElement, config: Partial<LockModeConfig> = {}) {
   ...
   */
  on(event: keyof LockModeCallbacks, callback: LockModeCallbacks[keyof LockModeCallbacks]) {
   ...
   */
  lock(level = LOCK_LEVELS.FULL, actions: string[] = []) {
   ...
   */
  unlock(password: string | null = null) {
   ...
   */
  attemptAction(action: string, callback?: (() => void) | null) {
   ...
   * @param {string} action 
   */
  _showBlockedMessage(action: string) {

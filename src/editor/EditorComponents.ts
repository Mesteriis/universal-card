...
type EditorComponentConfig = Record<string, any>;
...
export class EditorComponent {
  _config: EditorComponentConfig;
  _element: HTMLElement | null;

  constructor(config: EditorComponentConfig = {}) {
    this._config = config;
    this._element = null;
  }
...
export class TextInput extends EditorComponent {
  ...
  override setValue(value: string | null | undefined) {
    if (this._input) {
...
export class NumberInput extends EditorComponent {
  ...
  override setValue(value: number | null | undefined) {
    if (this._input) {
...
export class EntityPicker extends EditorComponent {
  _picker: PickerElement | null = null;
  ...
  override render() {
    ...
    this._picker = this._element.querySelector('ha-entity-picker') as PickerElement | null;
    this._picker?.addEventListener('value-changed', (e: Event) => {
      const value = (e as CustomEvent<{ value?: string }>).detail?.value || '';
      fireEvent(this._element, 'value-changed', { value });
    });
...
export class IconPicker extends EditorComponent {
  ...
  override render() {
    ...
    this._picker = this._element.querySelector('ha-icon-picker') as PickerElement | null;
    this._picker?.addEventListener('value-changed', (e: Event) => {
      const value = (e as CustomEvent<{ value?: string }>).detail?.value || '';
      fireEvent(this._element, 'value-changed', { value });
    });
...
export class ColorPicker extends EditorComponent {
  _colorInput: HTMLInputElement | null = null;
  _textInput: HTMLInputElement | null = null;
  ...
  override render() {
    ...
    this._colorInput = this._element.querySelector('.uc-editor-color-input') as HTMLInputElement | null;
    this._textInput = this._element.querySelector('.uc-editor-color-text') as HTMLInputElement | null;

    this._colorInput?.addEventListener('input', () => {
      if (!this._colorInput || !this._textInput) return;
      this._textInput.value = this._colorInput.value;
      fireEvent(this._element, 'value-changed', { value: this.getValue() });
    });

    this._textInput?.addEventListener('input', () => {
      if (!this._textInput || !this._colorInput) return;
...
export class Section extends EditorComponent {
  ...
  override render() {
    ...
    this._header = this._element.querySelector('.uc-editor-section-header') as HTMLElement | null;
    this._content = this._element.querySelector('.uc-editor-section-content') as HTMLElement | null;
   ...
   */
  addContent(content: HTMLElement | EditorComponent) {
  ...
  override setValue(_value: unknown) {}
}
...
export class ActionEditor extends EditorComponent {
  _typeSelect: HTMLSelectElement | null = null;
  _optionsContainer: HTMLElement | null = null;
  ...
  override render() {
    ...
    this._typeSelect = this._element.querySelector('.uc-editor-action-type') as HTMLSelectElement | null;
    this._optionsContainer = this._element.querySelector('.uc-editor-action-options') as HTMLElement | null;
  ...
  _updateOptions() {
    if (!this._typeSelect || !this._optionsContainer) return;
  ...
  override getValue() {
    if (!this._typeSelect || !this._optionsContainer) {
      return null;
    }

    const type = this._typeSelect.value;
    const result: Record<string, any> = { action: type };

    switch (type) {
      case 'call-service': {
        const service = (this._optionsContainer.querySelector('.action-service') as HTMLInputElement | null)?.value;
        const dataText = (this._optionsContainer.querySelector('.action-data') as HTMLTextAreaElement | null)?.value;
      ...
      case 'navigate': {
        const path = (this._optionsContainer.querySelector('.action-path') as HTMLInputElement | null)?.value;
        if (path) result.navigation_path = path;
        break;
      }
      case 'url': {
        const url = (this._optionsContainer.querySelector('.action-url') as HTMLInputElement | null)?.value;
  ...
  override setValue(value: Record<string, any> | null | undefined) {

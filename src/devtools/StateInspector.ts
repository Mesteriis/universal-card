...
type StateInspectorConfig = {
  ...
  watchHass: boolean;
  watchConfig: boolean;
  watchInternal: boolean;
  highlightChanges: boolean;
  changeHighlightDuration: number;
};
...
type StateWatcher = (currentValue: unknown, previousValue: unknown, changedPath: string) => void;
...
export class StateInspector {
   ...
   */
  constructor(cardElement: HTMLElement, config: Partial<StateInspectorConfig> = {}) {
   ...
   * @param {*} value 
   */
  setInternal(key: string, value: unknown) {
   ...
   */
  watch(path: string, callback: StateWatcher) {
   ...
   * @param {Function} callback 
   */
  unwatch(path: string, callback: StateWatcher) {
   ...
   */
  _checkChanges(changedPath: string) {
   ...
   * @returns {*}
   */
  _getByPath(obj: any, path: string) {
   ...
   * @param {*} value 
   */
  _setByPath(obj: Record<string, any>, path: string, value: unknown) {
  ...
  showPanel() {
    ...
    (this._panel.querySelector('.uc-inspector-search') as HTMLInputElement | null)?.addEventListener('input', (e) => {
      this._renderPanel((e.target as HTMLInputElement | null)?.value || '');
    });
  ...
  _renderPanel(searchQuery = '') {
    ...
    const activeTab = (this._panel.querySelector('.uc-inspector-tab.active') as HTMLElement | null)?.dataset.tab || 'config';
    const content = this._panel.querySelector('.uc-inspector-content') as HTMLElement | null;
    if (!content) return;
   ...
   * @param {string} changedPath 
   */
  _updatePanel(changedPath: string) {
   ...
   * @returns {string}
   */
  _renderObject(obj: any, path = '', searchQuery = '', depth = 0) {
   ...
   * @returns {string}
   */
  _renderPrimitive(value: unknown, path: string) {
   ...
   * @param {*} value 
   */
  _setValue(path: string, value: unknown) {
  ...
  _copyState() {
    ...
    navigator.clipboard.writeText(state).then(() => {
      ...
      const btn = this._panel?.querySelector('.uc-inspector-copy') as HTMLElement | null;
      if (!btn) return;

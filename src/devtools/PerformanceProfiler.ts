...
export class PerformanceProfiler {
  _config: PerformanceProfilerConfig;
  _metrics: Map<string, MetricBucket>;
  _activeTimers: Map<string, ActiveTimer>;
  _panel: HTMLElement | null;
  _isVisible: boolean;
  _memoryInterval: number | null;
  _fpsHistory: FpsSample[];
  _lastFrameTime: number;
  _boundRuntimeMetricHandler: (event: Event) => void;
  _isEnabled: boolean;
  _panelUpdateInterval: number | null;
  ...
  constructor(config: Partial<PerformanceProfilerConfig> = {}) {
    ...
    this._boundRuntimeMetricHandler = this._handleRuntimeMetric.bind(this);
  ...
  enable() {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('universal-card-performance', this._boundRuntimeMetricHandler);
    }
  ...
  disable() {
    ...
    if (typeof window !== 'undefined') {
      window.removeEventListener('universal-card-performance', this._boundRuntimeMetricHandler);
    }
   ...
   */
  start(name: string, type = METRIC_TYPES.RENDER) {
    if (!this._config.enabled) {
      return () => {};
    }
   ...
   * @returns {Promise<*>}
   */
  async measure(name: string, type: string, fn: () => Promise<any>) {
   ...
   * @returns {*}
   */
  measureSync(name: string, type: string, fn: () => any) {
    const end = this.start(name, type);
    try {
      return fn();
    } finally {
   ...
   * @param {Object} [meta]
   */
  record(type: string, name: string, duration: number, meta: Record<string, any> = {}) {
    if (!this._config.enabled) return;
    if (!Number.isFinite(duration)) return;
    this._recordMetric(type, name, duration, meta);
  }

  /**
   * Обрабатывает метрику, присланную из runtime через событие
   *
   * @private
   * @param {CustomEvent} event
   */
  _handleRuntimeMetric(event: Event) {
    if (!this._config.enabled) return;

    const detail = (event as CustomEvent<RuntimeMetricDetail>)?.detail || {};
    if (!detail.type || !Number.isFinite(detail.durationMs)) return;

    this._recordMetric(
      detail.type,
      detail.card_id || detail.name || detail.type,
      detail.durationMs,
      detail
    );
  }
   ...
   * @param {number} duration 
   */
  _recordMetric(type: string, name: string, duration: number, meta: Record<string, any> = {}) {
  ...
  _startMemoryTracking() {
    ...
    this._stopMemoryTracking();
      this._memoryInterval = window.setInterval(() => {
  ...
  _updatePanel() {
    ...
    // Summary
    const summary = this._panel.querySelector('.uc-profiler-summary') as HTMLElement | null;
    ...
    // Charts
    const charts = this._panel.querySelector('.uc-profiler-charts') as HTMLElement | null;
    if (!charts) return;
    charts.innerHTML = `
    ...
    // Details
    const details = this._panel.querySelector('.uc-profiler-details') as HTMLElement | null;
    if (!details) return;
   ...
   * @returns {string}
   */
  _renderBars(samples: Array<{ duration: number; name: string }>) {
   ...
   * @returns {string}
   */
  _renderLineChart(values: number[], targetMax = 60) {

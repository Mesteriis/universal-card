...
type LoggerListener = (type: string, entry: LoggerEntry | null) => void;
...
export class EventLogger {
  _config: EventLoggerConfig;
  _entries: LoggerEntry[];
  _listeners: Set<LoggerListener>;
  _groupedEvents: Map<string, GroupedEvent>;
   ...
   */
  log(level: string, category: string, message: string, data: Record<string, any> = {}) {
    ...
    const entry: LoggerEntry = {
      ...
      message,
      data,
      stack: level === LOG_LEVELS.ERROR ? new Error().stack || null : null,
      count: 1
    };
   ...
   * @param {Object} data 
   */
  lifecycle(event: string, data: Record<string, any> = {}) {
   ...
   * @param {*} newState 
   */
  stateChange(entityId: string, oldState: unknown, newState: unknown) {
   ...
   * @param {Object} entry 
   */
  _logToConsole(entry: LoggerEntry) {
   ...
   */
  addListener(callback: LoggerListener) {
   ...
   * @param {Function} callback 
   */
  removeListener(callback: LoggerListener) {
   ...
   * @param {Object} entry 
   */
  _notifyListeners(type: string, entry: LoggerEntry | null) {
   ...
   * @returns {Object[]}
   */
  getEntries(filter: LoggerFilter = {}) {
  ...
  _renderEntries() {
    ...
    const container = this._panel.querySelector('.uc-logger-entries') as HTMLElement | null;
    const levelFilter = (this._panel.querySelector('.uc-logger-filter-level') as HTMLSelectElement | null)?.value || '';
    const categoryFilter = (this._panel.querySelector('.uc-logger-filter-category') as HTMLSelectElement | null)?.value || '';
    if (!container) return;
...
let instance: EventLogger | null = null;

...
type RadialMenuItem = Record<string, any>;
...
type RadialMenuContext = Record<string, any>;
type Point = {
  x: number;
  y: number;
};
type RadialMenuPointerEvent = TouchEvent | MouseEvent;
type RadialMenuBoundHandler = EventListener;
...
export class RadialMenu {
  _config: RadialMenuConfig;
  _options: Required<Pick<RadialMenuOptions, 'radius' | 'itemSize' | 'startAngle'>> & RadialMenuOptions;
  _menu: HTMLDivElement | null;
  _center: HTMLDivElement | null;
  _hass: HomeAssistantLike;
  _selectedIndex: number | null;
  _centerPos: Point;
  _moveHandler: RadialMenuBoundHandler;
  _endHandler: RadialMenuBoundHandler;
   ...
   */
  constructor(config: RadialMenuConfig = {}, options: RadialMenuOptions = {}) {
   ...
   */
  show(x: number, y: number, context: RadialMenuContext = {}) {
   ...
   */
  _createMenu(context: RadialMenuContext): HTMLDivElement {
   ...
   */
  _createItem(item: RadialMenuItem, index: number, angleStep: number): HTMLButtonElement {
   ...
   */
  _handleMove(event: RadialMenuPointerEvent) {
    ...
    let y: number;
    if ('touches' in event) {
      x = event.touches[0]?.clientX ?? this._centerPos.x;
      y = event.touches[0]?.clientY ?? this._centerPos.y;
    } else {
      x = event.clientX;
      y = event.clientY;
    }
   ...
   */
  _handleEnd(event: RadialMenuPointerEvent) {
    this.hide(true);
  }
   ...
   * @param {Object} item - Item configuration
   */
  _executeItem(item: RadialMenuItem) {
    // Execute action
    if (item.action) {

...
import type {
  ContextMenuActionItemConfig,
  ContextMenuConfig,
  ContextMenuHandlerContext,
  ContextMenuItemConfig,
  ContextMenuSeparatorItemConfig
} from '../core/config-contracts.js';

type ContextMenuOptions = Record<string, never>;
type ContextMenuContext = ContextMenuHandlerContext;
type ContextMenuBoundHandler = (event: Event | KeyboardEvent) => void;
...
export class ContextMenu {
  ...
  _menu: HTMLDivElement | null;
  _hass: HomeAssistantLike;
  _closeHandler: ContextMenuBoundHandler;
  _escapeHandler: ContextMenuBoundHandler;
   ...
   */
  constructor(config: ContextMenuConfig = {}, options: ContextMenuOptions = {}) {
   ...
   */
  show(x: number, y: number, context: ContextMenuContext = {}) {
   ...
   */
  _createMenu(context: ContextMenuContext): HTMLDivElement {
    ...
    items.forEach((item, index) => {
      if (isContextMenuSeparatorItem(item)) {
   ...
   */
  _createMenuItem(item: ContextMenuActionItemConfig, index: number, context: ContextMenuContext): HTMLButtonElement {
   ...
   * @param {number} y - Y coordinate
   */
  _positionMenu(x: number, y: number) {
    if (!this._menu) return;
   ...
   * @param {Object} context - Context data
   */
  _showSubmenu(parentItem: HTMLElement, items: ContextMenuItemConfig[], context: ContextMenuContext) {
   ...
   * @param {Object} context - Context data
   */
  _handleItemClick(item: ContextMenuActionItemConfig, context: ContextMenuContext) {
    // Hide menu
    this.hide();
  ...
  _handleOutsideClick(event: Event) {
    if (this._menu && event.target instanceof Node && !this._menu.contains(event.target)) {
      this.hide();
    }
  }
...
function isContextMenuSeparatorItem(item: ContextMenuItemConfig): item is ContextMenuSeparatorItemConfig {

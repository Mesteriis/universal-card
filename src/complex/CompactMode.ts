...
type CompactLevel = typeof COMPACT_LEVELS[keyof typeof COMPACT_LEVELS];
type CompactTrigger = typeof COMPACT_TRIGGERS[keyof typeof COMPACT_TRIGGERS];
type CompactBreakpointMap = Record<CompactLevel, number>;
type CompactLevelSettings = {
  padding: string;
  gap: string;
  fontSize: string;
  iconSize: string;
...
type CompactModeConfig = {
  ...
  breakpoints?: Partial<CompactBreakpointMap>;
  containerBreakpoints?: Partial<CompactBreakpointMap>;
  levelSettings?: Partial<Record<CompactLevel, Partial<CompactLevelSettings>>>;
};

type ResolvedCompactModeConfig = {
  enabled: boolean;
  level: CompactLevel;
  trigger: CompactTrigger;
  breakpoints: CompactBreakpointMap;
  containerBreakpoints: CompactBreakpointMap;
  levelSettings: Record<CompactLevel, CompactLevelSettings>;
};

type MediaQueryListenerEntry = {
  mql: MediaQueryList;
...
export class CompactMode {
  ...
  _config: ResolvedCompactModeConfig;
  _currentLevel: CompactLevel;
  _resizeObserver: ResizeObserver | null;
  _mediaQueryListeners: MediaQueryListenerEntry[];
  _callbacks: Set<(newLevel: CompactLevel, oldLevel: CompactLevel) => void>;

  /**
   * @param {HTMLElement} element - Элемент карточки
   * @param {Object} config - Конфигурация
   */
  constructor(element: HTMLElement, config: CompactModeConfig = {}) {
    this._element = element;
    this._config = this._mergeConfig(
      DEFAULT_CONFIG as Record<string, unknown>,
      config as Record<string, unknown>
    ) as ResolvedCompactModeConfig;
   ...
   * @returns {Object}
   */
  _mergeConfig(defaults: Record<string, unknown>, overrides: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const key of Object.keys(overrides)) {
      ...
      if (overrideValue && typeof overrideValue === 'object' && !Array.isArray(overrideValue)) {
        result[key] = this._mergeConfig(
          (typeof defaultValue === 'object' && defaultValue ? defaultValue : {}) as Record<string, unknown>,
  ...
  _handleResize(width: number) {
    ...
    let newLevel: CompactLevel = COMPACT_LEVELS.NONE;

    // Находим подходящий уровень
    for (const [level, threshold] of Object.entries(breakpoints) as Array<[CompactLevel, number]>) {
      if (width <= threshold) {
        newLevel = level;
      }
    }
   ...
   * @returns {Object}
   */
  getCurrentSettings(): CompactLevelSettings {
    return this._config.levelSettings[this._currentLevel] || this._config.levelSettings[COMPACT_LEVELS.NONE];
  }
   ...
   * @param {string} level 
   */
  _applyLevel(level: CompactLevel) {
   ...
   * @returns {boolean}
   */
  shouldShow(elementType: string): boolean {
    const settings = this.getCurrentSettings();
    const key = `show${elementType.charAt(0).toUpperCase() + elementType.slice(1)}` as keyof CompactLevelSettings;
   ...
   * @returns {number}
   */
  getGridColumns(defaultColumns = 2): number {
    const settings = this.getCurrentSettings();
   ...
   */
  subscribe(callback: (newLevel: CompactLevel, oldLevel: CompactLevel) => void) {
   ...
   */
  _notifyCallbacks(newLevel: CompactLevel, oldLevel: CompactLevel) {

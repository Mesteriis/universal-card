...
type GroupingStrategy = typeof GROUPING_STRATEGIES[keyof typeof GROUPING_STRATEGIES];
type GroupSortOrder = typeof SORT_ORDER[keyof typeof SORT_ORDER];

type GroupAttributeRule = {
  name?: string;
...
type GroupRuleConfig = {
  ...
  state?: string | string[];
  attribute?: GroupAttributeRule;
};

type CustomGroupConfig = {
  name?: string;
  icon?: string;
  collapsed?: boolean;
...
type HomeAssistantLike = {
  states?: Record<string, Omit<GroupEntity, 'entity_id'> & Partial<Pick<GroupEntity, 'entity_id'>>>;
...
type AutoGroupingConfig = {
  ...
  custom_groups?: Record<string, CustomGroupConfig>;
};
...
export class AutoGrouping {
  ...
  _config: AutoGroupingConfig;
  _cache: AutoGroup[] | null;
   ...
   */
  constructor(hass: HomeAssistantLike | null, config: AutoGroupingConfig = {}) {
    this._hass = hass;
   ...
   * @param {Object} config 
   */
  setConfig(config: AutoGroupingConfig) {
    this._config = { ...DEFAULT_CONFIG, ...config };
    this._invalidateCache();
  }
   ...
   */
  getGroups(entityIds: string[] | null = null): AutoGroup[] {
   ...
   * @returns {Object[]}
   */
  _getFilteredEntities(entityIds: string[] | null): GroupEntity[] {
      ...
      if (entityLabels.length === 0) {
        ...
        for (const labelId of entityLabels) {
          if (!groups.has(labelId)) {
            const label: LabelRegistryEntry = labels[labelId] || { name: labelId, icon: 'mdi:label' };
            groups.set(labelId, {
              id: labelId,
              name: label.name,

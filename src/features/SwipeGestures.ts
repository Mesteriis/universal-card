...
type SwipeStartPoint = SwipePoint & {
  time: number;
};
type SwipeData = {
  deltaX: number;
...
type SwipeListener = (data: SwipeData) => void;
type SwipeListenerMap = Record<SwipeDirection, SwipeListener[]>;
...
export class SwipeGestures {
  ...
  _options: Required<Pick<SwipeOptions, 'threshold' | 'velocityThreshold' | 'direction' | 'preventScroll'>> & SwipeOptions;
  _touchStart: SwipeStartPoint;
  _touchCurrent: SwipePoint;
  _isTracking: boolean;
  _listeners: SwipeListenerMap;
   ...
   */
  _getSwipeDirection(deltaX: number, deltaY: number, velocityX: number, velocityY: number): SwipeDirection | null {
   ...
   */
  _triggerSwipe(direction: SwipeDirection, data: SwipeData) {
   ...
   */
  on(direction: SwipeDirection | string, callback: SwipeListener) {
    if (!this._listeners[direction]) {
      console.warn(`[UniversalCard] Invalid swipe direction: ${direction}`);
      return () => {};
    }
    
    this._listeners[direction as SwipeDirection].push(callback);
    
    // Return unsubscribe function
    return () => {
      const index = this._listeners[direction as SwipeDirection].indexOf(callback);
      if (index > -1) {
        this._listeners[direction as SwipeDirection].splice(index, 1);
      }
    };
  }
   ...
   * @returns {Function} Unsubscribe function
   */
  onSwipeLeft(callback: SwipeListener) {
   ...
   * @returns {Function} Unsubscribe function
   */
  onSwipeRight(callback: SwipeListener) {
   ...
   * @returns {Function} Unsubscribe function
   */
  onSwipeUp(callback: SwipeListener) {
   ...
   * @returns {Function} Unsubscribe function
   */
  onSwipeDown(callback: SwipeListener) {
